...
const api = {
  login: async (email, password) => {
    if (email === 'user@test.com' && password === 'password') {
      return { success: true, user: { id: '1', name: 'Test User', email, role: 'user' } };
    } else if (email === 'police@test.com' && password === 'password') {
      return { success: true, user: { id: '2', name: 'Police Officer', email, role: 'police' } };
    }
    throw new Error('Invalid credentials');
  },
  register: async () => ({ success: true }),
  uploadChallan: async () => ({ success: true }),
  getChallans: async () => [
    { id: '1', numberPlate: 'KA-01-AB-1234', description: 'Running red light',
      location: { address: 'MG Road, Bangalore' }, createdAt: new Date().toISOString(), status: 'pending', reportedBy: 'John Doe' },
    { id: '2', numberPlate: 'KA-02-CD-5678', description: 'Over speeding',
      location: { address: 'Whitefield, Bangalore' }, createdAt: new Date().toISOString(), status: 'pending', reportedBy: 'Jane Smith' }
  ]
};
    ...
    <div className="auth-container">
      <div className="auth-card">
        ...
        <div className="demo-credentials">
          ...
          <p>👮 Police: police@test.com / password</p>
        </div>
      </div>
    </div>
  );
...
const UploadChallan = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.uploadChallan({ image, numberPlate, description, location });
      onSuccess();
    } finally {
      setLoading(false);
    }
      ...
      <div className="container dashboard-content">
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card stat-purple">
            <div className="stat-icon">✅</div>
            <p className="stat-number">4,321</p>
...
const PoliceDashboard = () => {
  ...
  const handleAction = (challanId, action) => {
    setChallans(challans.map(c => c.id === challanId ? { ...c, status: action } : c));
          ...
          <div className="stat-card stat-blue">
            <div className="stat-content">
              <div>
                ...
                <p className="stat-number">{challans.length}</p>
              </div>
              <div className="stat-icon">📋</div>
            </div>
          </div>
          <div className="stat-card stat-yellow">
            <div className="stat-content">
              <div>
                ...
                <p className="stat-number">{challans.filter(c => c.status === 'pending').length}</p>
              </div>
              <div className="stat-icon">⏳</div>
            </div>
          </div>
          <div className="stat-card stat-green">
            <div className="stat-content">
              <div>
                ...
                <p className="stat-number">{challans.filter(c => c.status === 'approved').length}</p>
              </div>
              <div className="stat-icon">✅</div>
            </div>
          </div>
          <div className="stat-card stat-red">
            <div className="stat-content">
              <div>
                ...
                <p className="stat-number">{challans.filter(c => c.status === 'rejected').length}</p>
              </div>
              <div className="stat-icon">❌</div>
            </div>
          </div>
              ...
              {filteredChallans.map((challan) => (
                <div key={challan.id} className="challan-card">
                  ...
                  <div className="challan-meta">
                    ...
                    <p>👤 Reported by: {challan.reportedBy}</p>
                  </div>
